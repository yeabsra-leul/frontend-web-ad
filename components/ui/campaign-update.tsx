...
const schema = z.object({
      ...
      campaignBudget: z.number().min(0, { message: "Budget must be a positive number" }),      
      campaignStartDate: z.preprocess((arg) => arg === '' ? undefined : arg, z.date().refine((date) => date !== undefined, { message: "Start date is required" })),
      campaignEndDate: z.preprocess((arg) => arg === '' ? undefined : arg, z.date().refine((date) => date !== undefined, { message: "End date is required" })),
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
        const validatedData = schema.safeParse({
            ...
            campaignStartDate: formData.start ? new Date(formData.start) : undefined,            
  ...
  return (
    <form onSubmit={handleSubmit}>      
        ...
        <div className="w-full max-w-[60%] space-y-6">
          ...
          <div className="md:flex md:items-center mb-6">
              ...
              <div className="md:w-3/4">
                <input id="campaignName" name="name" aria-describedby="name-error" placeholder="Enter name" type="text"
                value={formData.name}
            ...
            <div className="md:flex md:items-center mb-6">
              ...
              <div className="md:w-3/4">
                <div id="name-error" aria-live="polite" aria-atomic="true">
                    {errors.campaignName && <p className="-mt-6 text-sm text-red-500">{errors.campaignName}</p>}
                </div>
              </div>
            </div>
            <div className="md:flex md:items-center mb-6">
              ...
              <div className="md:w-3/4">
                <input id="campaignBudget" name="budget" placeholder="$" aria-describedby="budget-error" 
                className="appearance-none border-2 border-gray-200 rounded w-full py-2 px-4 text-gray-700 leading-tight focus:outline-none focus:bg-white focus:border-blue-500" type="text"
            ...
            <div className="md:flex md:items-center mb-6">
              ...
              <div className="md:w-3/4">
                <div id="budget-error" aria-live="polite" aria-atomic="true">
                    {errors.campaignBudget && <p className="-mt-6 text-sm text-red-500">{errors.campaignBudget}</p>}
                </div>
              </div>
            </div>
            <div className="md:flex md:items-center mb-6">
              ...
              <div className="md:w-3/4">
                <Textarea id="campaignNotes" name="notes" placeholder="Campaign notes" value={formData.notes}
                className=""
                onChange={handleChange} />
              </div>
            </div>                
              ...
              <div className="md:w-3/4 inline-flex">
                <div className="md:w-1/2 inline-flex">
                  <div id="start-error" aria-live="polite" aria-atomic="true">
                    {errors.campaignStartDate && <p className="-mt-6 text-sm text-red-500">{errors.campaignStartDate}</p>}                    
                  </div>
                </div>
                <div className="md:w-1/2 inline-flex">
                  <div id="end-error" aria-live="polite" aria-atomic="true">
                  {errors.campaignEndDate && <p className="-mt-6 text-sm text-red-500">{errors.campaignEndDate}</p>}  
      ...
      <footer className="flex items-center justify-center px-6">
        ...
        <Button type="submit" color="primary" >Save Update</Button>
        </div>
      </footer>
    </form>
  );
